...
# -- Core settings --
SUBSCRIPTION_ID="<your-subscription-id>"          # Valero subscription
RESOURCE_GROUP="rg-dns-poc"                        # POC resource group
LOCATION="southcentralus"                          # Closest Azure region to Valero HQ (San Antonio)

# -- DNS settings --
PUBLIC_ZONE="poc.valero.com"                       # Public DNS zone for POC
PRIVATE_ZONE="poc-internal.valero.local"           # Private DNS zone for POC
...
ZONE_FILE_2="./zone-files/valero-zone2.zone"       # Second Bind zone file
ZONE_NAME_1="zone1.poc.valero.com"                 # Azure zone name for file 1
...
EVENTHUB_NAMESPACE="eh-dns-poc"                    # Event Hub namespace name
EVENTHUB_NAME="dns-query-logs"                     # Event Hub name
EVENTHUB_SKU="Basic"                               # Basic is fine for POC
LOG_ANALYTICS_WORKSPACE="la-dns-poc"               # Log Analytics workspace for reporting

# -- Zone Snapshots --
SNAPSHOT_DIR="./zone-snapshots"                    # Directory to store zone export files

# -- Traffic Manager (Geo routing) --
TM_PROFILE_NAME="tm-valero-geo-poc"
TM_DNS_NAME="valero-geo-poc"                       # Must be globally unique
US_ENDPOINT_IP="<us-endpoint-ip>"                  # US-facing IP
UK_ENDPOINT_IP="<uk-endpoint-ip>"                  # UK-facing IP

echo "============================================"
echo " Valero Azure DNS POC — Configuration Loaded"
echo "============================================"
...
#   named-checkzone internal.valero.com /etc/bind/zones/db.internal > valero-zone2.zone
#
...
az network dns record-set txt add-record \
  -g "$RESOURCE_GROUP" -z "$PUBLIC_ZONE" \
  -n "test-txt" -v "v=spf1 include:valero.com ~all"
...
{
  "Name": "DNS Record Operator - Valero POC",
  "IsCustom": true,
...
az role assignment create \
  ...
  --role "DNS Record Operator - Valero POC" \
...
echo "  az network dns zone create -g $RESOURCE_GROUP -n 'operator-should-fail.valero.com'"
echo ""
...
echo "  az network dns zone create -g $RESOURCE_GROUP -n 'admin-test-zone.valero.com'"
echo ""
echo "  # Should SUCCEED — admin can delete zones"
echo "  az network dns zone delete -g $RESOURCE_GROUP -n 'admin-test-zone.valero.com' --yes"
echo ""
...
# --------------------------------------------------------------------------
#
# Valero uses DigiCert CertCentral. DigiCert DCV uses _dnsauth TXT records
# (not _acme-challenge). This section tests BOTH conventions:
#   - _dnsauth (DigiCert CertCentral — Valero's production workflow)
...
# ==========================================================================
# TEST 2: Subdomain DCV — Certificate for app.poc.valero.com
# ==========================================================================
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
# ==========================================================================
# TEST 3: Wildcard DCV — Certificate for *.poc.valero.com
# ==========================================================================
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
# ==========================================================================
# TEST 7: DigiCert DCV — _dnsauth TXT Record (Valero's Production Flow)
# ==========================================================================
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
echo " This is Valero's PRODUCTION workflow with DigiCert"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
# ==========================================================================
# TEST 8: DigiCert DCV — Subdomain _dnsauth (app.poc.valero.com)
# ==========================================================================
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
# ==========================================================================
# TEST 9: DigiCert DCV — Wildcard _dnsauth (*.poc.valero.com)
# ==========================================================================
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
if [ -n "$CURRENT_USER" ]; then
  ...
  az role assignment create \
    ...
    --scope "/subscriptions/$SUBSCRIPTION_ID/resourceGroups/$RESOURCE_GROUP/providers/Microsoft.KeyVault/vaults/kv-valero-dns-poc" \
...
echo "     --vault-name kv-valero-dns-poc \\"
echo "     --name poc-valero-com \\"
echo "     --file cert.pfx"
echo ""
...
echo "   portal.azure.com → Key vaults → kv-valero-dns-poc → Certificates"
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
# --------------------------------------------------------------------------
# If Valero prefers PowerShell, here's a bulk creation script:
...
$records = @(
    ...
    @{ Name = "api";     Type = "CNAME";  Value = "web1.poc.valero.com" }
    @{ Name = "mail";    Type = "MX";     Value = "mail.poc.valero.com"; Priority = 10 }
    @{ Name = "spf";     Type = "TXT";    Value = "v=spf1 include:valero.com ~all" }
)
...
az monitor diagnostic-settings create \
  --name "dns-query-logs-to-eventhub" \
...
echo ""
echo "Valero uses IBM QRadar. Follow these steps to connect:"
echo ""
...
# ============================================================================
# Required by Valero: ability to take point-in-time zone backups and restore.
...
SNAPSHOT_TEST_ZONE="snapshot-test.poc.valero.com"
...
echo ""
echo '  SNAPSHOT_FILE="./zone-snapshots/snapshot-poc.valero.com-$(date +%Y%m%d).zone"'
echo '  az network dns zone export -g rg-dns-poc -n poc.valero.com -f "$SNAPSHOT_FILE"'
...
# ============================================================================
# Required by Valero: operational dashboards for DNS query volumes,
...
echo "  3. Save as 'Valero DNS POC - Operations Dashboard'"
echo ""
...
# ============================================================================
# Optional for Valero but included in their scorecard.
...
az network traffic-manager profile create \
  --resource-group "$RESOURCE_GROUP" \
  --name "tm-valero-failover-poc" \
  --routing-method Priority \
  --unique-dns-name "valero-failover-poc" \
  --ttl 30 \
  --protocol HTTP \
  --port 80 \
  --path "/" \
...
az network traffic-manager endpoint create \
  --resource-group "$RESOURCE_GROUP" \
  --profile-name "tm-valero-failover-poc" \
...
echo "  dig valero-failover-poc.trafficmanager.net +short"
dig valero-failover-poc.trafficmanager.net +short
...
FAILOVER_RESULT=$(dig valero-failover-poc.trafficmanager.net +short 2>/dev/null)
...
# ============================================================================
# These are OPTIONAL / nice-to-have per Valero's confirmed scorecard.
...
az network traffic-manager profile create \
  --resource-group "$RESOURCE_GROUP" \
  --name "tm-valero-weighted-poc" \
  --routing-method Weighted \
  --unique-dns-name "valero-weighted-poc" \
  --ttl 30 \
  --protocol HTTP \
  --port 80 \
  --path "/" \
...
az network traffic-manager endpoint create \
  --resource-group "$RESOURCE_GROUP" \
  --profile-name "tm-valero-weighted-poc" \
  --name "primary" \
  --type externalEndpoints \
  --target "$US_ENDPOINT_IP" \
  --weight 70 \
...
echo "  for i in \$(seq 1 20); do dig valero-weighted-poc.trafficmanager.net +short; done"
...
echo "    - Key Vault (kv-valero-dns-poc)"
...
echo ""
echo "  az role definition delete --name 'DNS Record Operator - Valero POC'"
echo ""
...
echo "  az network traffic-manager profile show -g $RESOURCE_GROUP -n tm-valero-failover-poc -o table"
echo ""
echo "Check endpoint status:"
echo "  az network traffic-manager endpoint list -g $RESOURCE_GROUP --profile-name tm-valero-failover-poc -o table"
echo ""
echo "========================================"
