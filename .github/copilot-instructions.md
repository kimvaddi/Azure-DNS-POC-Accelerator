...
| `dns-operator-role.json` | Custom RBAC role definition for DNS Record Operator — 13 Actions, 7 NotActions, MS Learn validated |
| `Zava_RBAC_Delegation.ps1` | Standalone RBAC script — creates custom role, assigns users, runs verification tests, applies resource lock |
...
- RBAC uses `DNS Zone Contributor` (built-in, for DNS admins) and a custom `DNS Record Operator` role (for operators).
- RBAC two-role model: DNS Zone Contributor (built-in) scoped to RG vs DNS Record Operator (custom) scoped to individual DNS zone.
- Custom role blocks: zone write/delete, SOA/NS modification, DNSSEC config changes. Allows: all record type CRUD (A, AAAA, CNAME, MX, TXT, SRV, CAA, PTR).
- Resource lock (CanNotDelete) on DNS zone prevents accidental deletion even by Admins.
- Record-set level RBAC supported: scope to individual record sets (e.g., `Microsoft.Network/dnsZones/<zone>/TXT/_acme-challenge`).
